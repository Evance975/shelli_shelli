...
int my_find_program(data_of_program *data)
{
	...
	free(data->tokens[0]);
	data->tokens[0] = my_str_concat(my_str_duplicate("/"), data->command_name);
	if (!data->tokens[0])
		return (2);
	...
	for (q = 0; directories[q]; q++)
	{
		...
		directories[q] = my_str_concat(directories[q], data->tokens[0]);
		...
		if (ret_code == 0 || ret_code == 126)
		{
			...
			errno = 0;
			free(data->tokens[0]);
			data->tokens[0] = my_str_duplicate(directories[q]);
			my_free_array_of_pointers(directories);
			return (ret_code);
		}
	}
...
char **my_tokenize_path(data_of_program *data)
{
	...
	q = 0;
	tokens[q] = my_str_duplicate(my_strtok(PATH, ":"));
	while (tokens[q++])
	{
		tokens[q] = my_str_duplicate(my_strtok(NULL, ":"));
	}
