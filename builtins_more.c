...
{
    char *dir_home = my_env_get_key("HOME", data), *dir_old = NULL;
    char old_dir[128] = {0};
    int error_code = 0;

    if (data->tokens[1])
    {
        if (my_str_compare(data->tokens[1], "-", 0))
        {
            dir_old = my_env_get_key("OLDPWD", data);
            if (dir_old)
                error_code = my_set_work_directory(data, dir_old);
            _print(my_env_get_key("PWD", data));
            _print("\n");
...
int my_set_work_directory(data_of_program *data, char *new_dir)
{
    ...
    if (!my_str_compare(old_dir, new_dir, 0))
    {
        ...
        my_env_set_key("PWD", new_dir, data);
    }
    my_env_set_key("OLDPWD", old_dir, data);
    return (0);
}
...
int my_builtin_help(data_of_program *data)
{
    ...
    {
        length = my_str_length(data->tokens[1]);
        if (my_str_compare(data->tokens[1], messages[q], length))
        {
