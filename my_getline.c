...
int my_getline(data_of_program *data)
{
        ...
        (array_operators[0] == '|' && errno == 0))
    {
        ...
        q = 0;
        do {
            array_commands[q] = my_str_duplicate(my_strtok(q ? NULL : buff, "\n;"));
            /*checks the presence of && and || operators*/
            /*and splits them*/
            q = my_check_logic_ops(array_commands, q, array_operators);
    ...
    return (my_str_length(data->input_line));
}

/**
* my_check_logic_ops - function that checks for && and || operators
* and split them.
...
*/
int my_check_logic_ops(char *array_commands[], int q, char array_operators[])
{
    char *temp = NULL;
    int r;
    ...
    for (r = 0; array_commands[q] != NULL && array_commands[q][r]; r++)
    {
        if (array_commands[q][r] == '&' && array_commands[q][r + 1] == '&')
        {
            ...
            array_commands[q] = my_str_duplicate(array_commands[r]);
            array_commands[q + 1] = my_str_duplicate(temp + r + 2);
            q++;
            array_operators[q] = '&';
            free(temp);
            r = 0;
        ...
        if (array_commands[q][r] == '|' && array_commands[q][r + 1] == '|')
        {
            ...
            temp = array_commands[q];
            array_commands[q][r] = '\0';
            array_commands[q] = my_str_duplicate(array_commands[q]);
            array_commands[q + 1] = my_str_duplicate(temp + r + 2);
            q++;
            array_operators[q] = '|';
            free(temp);
