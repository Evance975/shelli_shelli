...
/**
 * str_length - outputs the length of string
...
/**
 * str_compare - does comparison of 2 strings
 * @string1: String 1, or shorter
 * @string2: String 2, or longer
...
char *str_concat(char *string1, char *string2)
{
	...
	/* duplicate of string1 */
	for (length1 = 0; string1[length1] != '\0'; length1++)
		result[length1] = string1[length1];
	free(string1);

	/* duplicate of string2 */
...
void str_reverse(char *string)
{
	...
	int q = 0, length = str_length(string) - 1;
	char hold;

	while (q < length)
	{
		hold = string[q];
		string[q++] = string[length];
		string[length--] = hold;
	}
}
