...
/**
* _getline - takes 1 line input from prompt.
...
int _getline(data_of_program *data)
{
		...
		(array_operators[0] == '|' && errno == 0))
	{
		...
		/* reading into buffer from the file descriptor */ /*CHANGE THE BELOW LINE OF CODE*/
		...
		q = 0;
		do {
			array_commands[q] = str_duplicate(_strtok(q ? NULL : buff, "\n;"));
...
* @i: array command index to be checked.
* @array_operators: variable representing the array
* of logical operators for each of previous command.
*
...
int check_logic_ops(char *array_commands[], int q, char array_operators[])
{
	...
	for (r = 0; array_commands[q] != NULL  && array_commands[q][r]; r++)
	{
		...
		{
			/* if && character is found; spliting line */
			temp = array_commands[q];
			array_commands[q][r] = '\0';
			array_commands[q] = str_duplicate(array_commands[r]);
			array_commands[q + 1] = str_duplicate(temp + r + 2);
			q++;
			array_operators[q] = '&';
			free(temp);
			r = 0;
		...
		if (array_commands[q][r] == '|' && array_commands[q][r + 1] == '|')
		{
			...
			temp = array_commands[q];
			array_commands[q][r] = '\0';
			array_commands[q] = str_duplicate(array_commands[q]);
			array_commands[q + 1] = str_duplicate(temp + r + 2);
			q++;
			array_operators[q] = '|';
			free(temp);
